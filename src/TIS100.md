...
* Bugs: 
  when processing inst we don't enter read state directly.
  we need to check if a value is available for reading
  if so we shouldn't enter read block state.
* for inst with src we should try to resolve it to num,
  only on failure, should we enter read state.
